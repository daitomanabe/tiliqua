...
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/tiliqua-dslx-adsr-test.XXXXXX")
trap 'rm -rf "$temporary_dir"' EXIT HUP INT TERM
actual_file="$temporary_dir/adsr_test.actual"

"$driver" dslx-fn-eval \
    --dslx_input_file "$gateware_dir/dslx/adsr.x" \
    --dslx_top adsr_step \
    --warnings_as_errors true \
    --input_ir_path "$gateware_dir/dslx/adsr_test.irvals" \
    > "$actual_file"

diff -u "$gateware_dir/dslx/adsr_test.expected" "$actual_file"
echo "DSLX ADSR vectors passed"
