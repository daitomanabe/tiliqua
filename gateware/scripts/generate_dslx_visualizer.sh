...
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/tiliqua-dslx-visualizer.XXXXXX")
trap 'rm -rf "$temporary_dir"' EXIT HUP INT TERM

source_file="$gateware_dir/dslx/visualizer.x"
ir_file="$temporary_dir/visualizer.ir"
verilog_file="$temporary_dir/tiliqua_dslx_visualizer.v"
output_file="$gateware_dir/dslx/generated/tiliqua_dslx_visualizer.v"
...
"$driver" dslx2pipeline \
    ...
    --delay_model unit \
    --pipeline_stages 1 \
    --module_name tiliqua_dslx_visualizer \
    --flop_inputs false \
    --flop_outputs false \
