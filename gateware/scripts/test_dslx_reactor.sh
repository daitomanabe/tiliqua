...
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/tiliqua-dslx-test.XXXXXX")
trap 'rm -rf "$temporary_dir"' EXIT HUP INT TERM
actual_file="$temporary_dir/reactor_test.actual"

"$driver" dslx-fn-eval \
    --dslx_input_file "$gateware_dir/dslx/reactor.x" \
    --dslx_top reactor_step \
    --warnings_as_errors true \
    --input_ir_path "$gateware_dir/dslx/reactor_test.irvals" \
    > "$actual_file"

diff -u "$gateware_dir/dslx/reactor_test.expected" "$actual_file"
echo "DSLX reactor vectors passed"
