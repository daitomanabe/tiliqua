...
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/tiliqua-dslx.XXXXXX")
trap 'rm -rf "$temporary_dir"' EXIT HUP INT TERM

source_file="$gateware_dir/dslx/reactor.x"
ir_file="$temporary_dir/reactor.ir"
verilog_file="$temporary_dir/tiliqua_dslx_reactor.v"
output_file="$gateware_dir/dslx/generated/tiliqua_dslx_reactor.v"
...
"$driver" dslx2pipeline \
    ...
    --delay_model unit \
    --pipeline_stages 1 \
    --module_name tiliqua_dslx_reactor \
    --flop_inputs false \
    --flop_outputs false \
